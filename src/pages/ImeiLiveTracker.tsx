...
export default function ImeiLiveTracker() {
  ...
  const startTracking = async (e: React.FormEvent) => {
    ...
    if (!validateImei(imei)) {
      setError('Invalid IMEI format. Please enter a valid 15-digit IMEI.');
      return;
    }

    setError(null);
    setIsSearching(true);
    setStatus('Initializing HLR/VLR handshake...');

    // Phase 1: Handshake
    await new Promise(r => setTimeout(r, 1000));
    setStatus('Querying global GSMA blacklist database...');
    
    // Phase 2: Signal Triangulation
    await new Promise(r => setTimeout(r, 1000));
    setStatus('Triangulating signal from nearest cell towers...');
      ...
      const id = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (!isTracking) {
            ...
            setTowerId(`CID-${Math.floor(Math.random() * 99999)}-LAC-${Math.floor(Math.random() * 999)}`);
            ...
            const intId = setInterval(() => {
              ...
              setSignalStrength(s => Math.max(40, Math.min(100, s + (Math.random() - 0.5) * 5)));
            }, 3000);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">IMEI Live Triangulation</h1>
          <p className="text-zinc-500 mt-1">Real-time hardware-level location tracking via cellular network pings.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-lg font-semibold">Target Acquisition</h2>
            </div>

            <form onSubmit={startTracking} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">
                  Target IMEI (15 Digits)
                </label>
                <input
                  type="text"
                  value={imei}
                  onChange={(e) => setImei(e.target.value.replace(/\D/g, '').slice(0, 15))}
                  placeholder="Enter IMEI to track..."
              ...
              {!isTracking ? (
                <button
                  type="submit"
                  disabled={isSearching || imei.length < 15}
              ...
              {status && (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500">System Status</span>
                  </div>
          ...
          {isTracking && (
            ...
            >
              <h3 className="text-sm font-medium text-zinc-500 uppercase tracking-wider">Network Data</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2">
                  <div className="flex items-center gap-2 text-zinc-500 mb-1">
                    ...
                    <span className="text-[10px] uppercase tracking-wider font-semibold">Active Cell Tower</span>
                  </div>
                  <p className="text-xs font-mono text-zinc-300 truncate">{towerId}</p>
                </div>

                <div className="pt-2">
                  <div className="flex items-center gap-2 text-zinc-500 mb-1">
                    <MapPin className="h-3 w-3" />
                    <span className="text-[10px] uppercase tracking-wider font-semibold">Est. Accuracy</span>
                  </div>
          ...
          <div className="h-[400px] md:h-[600px] lg:h-[700px] rounded-3xl overflow-hidden border border-white/5 bg-[#0d0d0d] relative">
            {!position ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-500 z-10 bg-[#0d0d0d]">
                ...
                <p className="text-sm">Awaiting Target Acquisition...</p>
                <p className="text-[10px] mt-2 uppercase tracking-widest opacity-50">Enter IMEI to begin triangulation</p>
              </div>
            ) : (
              <MapContainer
                center={position}
                ...
                <Marker position={position}>
                  <Popup>
                    <div className="text-xs">
                      <p className="font-bold text-emerald-600">TARGET LOCKED</p>
            ...
            {isTracking && (
              <div className="absolute top-6 right-6 z-10 flex flex-col gap-2">
                <div className="px-4 py-2 bg-[#0d0d0d]/80 backdrop-blur-md border border-emerald-500/20 rounded-xl flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">Live Feed Active</span>
                </div>
              </div>
            )}
          </div>
